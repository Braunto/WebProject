...
$sql="SELECT firstname, lastname, email, city, profileimage FROM members WHERE memberID=".$_GET['memberID'];
$result=$conn->query($sql);
if($row=$result->fetch_assoc()){
    //found the member -> display information
    echo '<br>';
      echo '<br>';
    echo '<h3 class="text-white mb-4">Nordic Guides - ';
	...
	if(empty($row['profileimage'])) echo '<img class="card-img-top" src="images/avatar.png" alt="Card image">';
	else echo '<img class="card-img-top" src="images/members/'.$row['profileimage'].'" style="width:300px;" alt="Card image">';
