...
session_start();
$sql="SELECT email FROM members WHERE memberID=".$_SESSION['userID'];
...
if($result->num_rows > 0){
    ...
    $row=$result->fetch_assoc();
        ...
        if($_POST['newemail1']==$_POST['newemail2']){
            ...
            if(!validEmail($email)) {
            $_SESSION['ema']='<p class="alert alert-info">Error Email.</p>';
                
            }else if($result2->num_rows > 0){
                
                
                $_SESSION['ema']='<p class="alert alert-danger">Email already exist!</p>';
                ...
                if($conn->query($sql)===TRUE){
                   $_SESSION['ema']='<p class="alert alert-success">Email updated!</p>';
                }else $_SESSION['ema']='<p class="alert alert-danger">Update failed!</p>';
            }          
        }else $_SESSION['ema']='<p class="alert alert-danger">New emails do not match!</p>';
    
}else $_SESSION['ema']='<p class="alert alert-danger">Could not retrieve user data!</p>';
